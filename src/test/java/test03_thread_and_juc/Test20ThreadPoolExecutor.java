 ...
 *
 */
public class Test20ThreadPoolExecutor {
	...
	public void test03() throws InterruptedException, ExecutionException {
		...
		*/
		...
		for (int i = 0; i < 10; i++) {
			int tmp = i;
			ScheduledFuture<Integer> future = pool3.schedule(new Callable<Integer>() {
				@Override
			...
			System.out.println(future.get()); //只有在调用future.get方法时才执行
		}
		pool3.shutdown();
		*/
	}
